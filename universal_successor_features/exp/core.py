...
def run_rl_second_phase(config = None, **kwargs):
    ...
    agent = config.agent.cls.load_from_checkpoint(my_env,
    ...
    while step <= config.n_steps:
        ...
        while not terminated and not truncated and step <= config.n_steps:
            ...
            log.add_value(config.log_name_done_rate,
                          done_rate
                          )
        ...
        log.add_value(config.log_name_step_per_episode,
                      step_per_episode
                      )
        log.add_value(config.log_name_reward_per_episode,
                      reward_per_episode
                      )
...
def step_feature_goal_agent(obs, agent, my_env, goals_so_far, training):
    ...
    transition = (obs["agent_position_features"],
                  obs["goal_position"],
                  action,
                  reward,
