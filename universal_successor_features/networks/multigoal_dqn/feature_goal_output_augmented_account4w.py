...
class FeatureGoalAugmentedDQN(torch.nn.Module):
    ...
    def __init__(self, config=None, **kwargs) -> None:
        ...
        self.layer_concat = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            torch.nn.Linear(
                in_features=self.config.H2_size, out_features=self.config.H3_size
            ),
            torch.nn.ReLU(),
            torch.nn.Linear(
                in_features=self.config.H3_size, out_features=self.config.num_actions
            ),
        )

    def forward(self, agent_position_features, env_goal_position, **kwargs):
        env_goal_representation = self.goal_layer(env_goal_position)
        joined_representation = torch.cat((agent_position_features, env_goal_representation), dim=1)
