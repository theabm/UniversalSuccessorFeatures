...
# In particular, it uses three additional hidden layers of size: 256, H2, H3,
# where H2 and H3 can be choosen in any of the combinations below.
# The rationale behind this is that, compared to USF architecture, the multigoalDQN has
# less parameters, thus, the comparison is not fair.
...
# 1. The head of the USF architecture is made up of a linear layer 256 to 4 * 100.
# Additionally, we have linear layers for the reward weights of 64 - 64 - 100.
# In total, this is 112896 parameters
# 2. The multigoal DQN instead, as described in the paper,
# only has a final layer of 256 - 4.
# To render the num of params equal, we add two additional
# hidden layers after 256 and need to solve:
...
#
#
# Please note: Using this augmented network makes sense only if the flow of information 
# in USF passes through the whole network (as described in the paper).
# If we were to keep separate networks for the weights and the SF, 
# this would not make sense anymore because the weights w
# are part of a different network.
