...
@pytest.mark.parametrize(
    ...
    [
        (nn.StateGoalRBFUSF, mem.ExperienceReplayMemory, 1100),
    ],
)
def test_training_sgrbf(network, memory, n_steps, seed=0):
    if seed is not None:
        eu.misc.seed(seed)
...
@pytest.mark.parametrize(
    "network, memory, n_steps",
    [
        (nn.StateGoalWeightRBFUSF, mem.ExperienceReplayMemory, 1000),
    ],
)
def test_training_sgwrbf(network, memory, n_steps, seed=0):
    if seed is not None:
        eu.misc.seed(seed)
    ...
    agent = a.StateGoalWeightRBFAgent(
        env=my_env,
        epsilon={"value": 1.0},
        train_for_n_iterations=2,
        discount_factor=0.5,
