...
def test_training(network, discount_factor = 0.5, num_episodes=50, seed=0):
    ...
    for i in range(my_env.rows):
        for j in range(my_env.columns):
            ...
            idx = i*my_env.rows + j
            if idx == 8:
                q_pred_g2_array.append(agent.policy_net(agent_position_features=agent_position_features, goal_position=goal_2_position, goal_weights = goal_2_weights)[0].cpu().squeeze().detach().numpy())
                continue
            elif idx == 6:
                q_pred_g1_array.append(agent.policy_net(agent_position_features=agent_position_features, goal_position=goal_1_position, goal_weights = goal_1_weights)[0].cpu().squeeze().detach().numpy())
                continue
            else:
                q_pred_g1_array.append(agent.policy_net(agent_position_features=agent_position_features, goal_position=goal_1_position, goal_weights = goal_1_weights)[0].cpu().squeeze().detach().numpy())
                q_pred_g2_array.append(agent.policy_net(agent_position_features=agent_position_features, goal_position=goal_2_position, goal_weights = goal_2_weights)[0].cpu().squeeze().detach().numpy())
