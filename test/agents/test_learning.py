...
def test_training(discount_factor = 0.5, num_episodes=50, seed=0):
    ...
    agent = a.StateGoalAgent(
        epsilon = {"value" : 1.0}, train_for_n_iterations = 2, discount_factor = discount_factor
        )
    device = agent.device
    ...
    goal_1_position = torch.tensor(goal_1_position).to(torch.float).to(device).unsqueeze(0)
    goal_2_position = torch.tensor(goal_2_position).to(torch.float).to(device).unsqueeze(0)
            
    q_pred_g1_array = []
    q_pred_g2_array = []
    for i in range(my_env.rows):
        for j in range(my_env.columns):
            agent_position = torch.tensor([i,j]).to(torch.float).to(device).unsqueeze(0)
            idx = i*my_env.rows + j
            if idx == 8:
