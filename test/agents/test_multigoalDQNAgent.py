...
def test_when_episode_starts_epsilon_defined():
    my_dqn = dqn.MultigoalDQNAgentBase()
...
def test_when_episode_starts_epsilon_is_correct_value(epsilon_decay_type, epsilon_expected):
    my_dqn = dqn.MultigoalDQNAgentBase(epsilon = epsilon_expected, epsilon_decay = {"type": epsilon_decay_type, "params": {"max": epsilon_expected}}) 
    my_dqn.start_episode(episode=0)
...
def test_decay_type(decay, epsilon_expected, num_episodes = 10):
    my_dqn = dqn.MultigoalDQNAgentBase(epsilon_decay = {"type":decay, "params": {"scheduled_episodes": num_episodes}})
...
def test_coherency_of_types():
    my_dqn = dqn.MultigoalDQNAgentBase(network = {"cls":mdqn.StateGoalPaperDQN})
    state = np.array([2,2])
    goal = np.array([5,5])
...
def test_choose_action():
    ...
    my_dqn = dqn.MultigoalDQNAgentBase(network = {"cls":mdqn.FeatureGoalPaperDQN})
    ...
    my_dqn = dqn.MultigoalDQNAgentBase(network = {"cls":udqn.StateGoalUSF})
    assert my_dqn.choose_action(s = np.random.rand(2), g = np.random.rand(2), purpose="testing") is not None
    my_dqn = dqn.MultigoalDQNAgentBase(network = {"cls":udqn.StateGoalWeightUSF})
    assert my_dqn.choose_action(s = np.random.rand(2), g = np.random.rand(2), w = np.random.rand(100), purpose="testing") is not None
    my_dqn = dqn.MultigoalDQNAgentBase(network = {"cls":udqn.FeatureGoalWeightUSF})
    assert my_dqn.choose_action(phi_s = np.random.rand(100), g = np.random.rand(2), w = np.random.rand(100), purpose="testing") is not None
    my_dqn = dqn.MultigoalDQNAgentBase(network = {"cls":udqn.FeatureGoalUSF})
    assert my_dqn.choose_action(phi_s = np.random.rand(100), g = np.random.rand(2), purpose="testing") is not None
